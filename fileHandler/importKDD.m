...
%% Convert and classifying attacks type column
normal = dataArray{:, 42};
for rows = 1:length(normal)
    value = normal(rows);
    disp(['classifying attacks, for row - ' num2str(rows) ''])
    arrayVal = attack2num(value);
    Attack_Status{rows} = arrayVal(1,:);   
end
...
filtData = [Duration_of_Conn,Source_Bytes,Dest_Bytes,Failed_Login];
